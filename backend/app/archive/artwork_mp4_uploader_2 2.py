...
human_list = set(
    1, 5, 7, 33, 37, 38, 40, 41, 51, 52, 54, 56, 57, 58, 59, 60, 63, 64, 70,
    71, 72, 73, 74, 75, 83, 88, 89, 94, 121, 122, 123, 124, 125, 126, 127,
...
# DB에서 Artwork 읽기
artworks = session.query(Artwork).filter(Artwork.id.in_(gcs_paths.keys())).all()[:1]
...
for num, artwork in enumerate(artworks):
    try:
        ...
        local_mp4_path = os.path.join(video_dir, f"{sanitized_title}_{num}.mp4")
